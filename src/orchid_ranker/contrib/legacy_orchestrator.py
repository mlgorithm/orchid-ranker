"""Legacy orchestrator implementation retained for backward compatibility.

Prefer :mod:`orchid_ranker.agents.agentic` for the supported API. This module
is kept in :mod:`orchid_ranker.contrib` so experiments depending on the older
signature continue to run."""
...
warnings.warn(
    "orchid_ranker.contrib.legacy_orchestrator is deprecated and will be removed in v1.0. "
...
_DEBUG_ORCH = os.getenv("ORCHID_DEBUG_ORCH", "0").lower() in {"1", "true", "yes", "on"}

def enable_debug_orch_logs(flag: bool = True) -> None:
    """Enable or disable legacy orchestrator debug logging."""
    global _DEBUG_ORCH
...
def _p(*args) -> None:
    if _DEBUG_ORCH or logger.isEnabledFor(logging.DEBUG):
