...
def evaluate_orchid(
    ...
    # Fit with memory measurement
    def fit_model():
        rec.fit(train, user_col="user_id", item_col="item_id")

    fit_model, fit_memory_mb = measure_memory(fit_model)
...
def evaluate_surprise(
    ...
    try:
        ...
        fit_algo, fit_memory_mb = measure_memory(fit_algo)
        t0 = time.perf_counter()
        fit_algo()
        fit_time = time.perf_counter() - t0
        ...
        # Inference
        users_to_eval = [u for u in ground_truth if u in trainset.to_raw_uid(u) for _ in [1]]
        users_to_eval = list(ground_truth.keys())[:50]
        infer_times = []
        recs_list = []
...
def evaluate_implicit(
    ...
    try:
        ...
        fit_model_fn, fit_memory_mb = measure_memory(fit_model_fn)
        t0 = time.perf_counter()
        fit_model_fn()
        fit_time = time.perf_counter() - t0
...
def main():
    ...
    # Orchid strategies
    orchid_strategies = ["als", "neural_mf", "popularity", "user_knn"]
