#!/usr/bin/env python3
"""End-to-end Last.fm 1K music benchmark runner.

Downloads data, preprocesses, trains all baselines (with grid search),
trains the engage simulator, runs replay sessions, computes all metrics,
and writes ``results.json`` + a Markdown report.

Usage::

    # Full benchmark
    PYTHONPATH=src python benchmarks/music/run.py

    # Quick smoke test (< 5 min, for CI)
    PYTHONPATH=src python benchmarks/music/run.py --smoke

See ``docs/roadmap/IMPLEMENTATION_PLAN.md`` and Appendix D for protocol.
"""
...
def _catalog_diversity(
    ...
    """Unique-item ratio and catalog coverage across all recommendations."""
    ...
    if not all_recommended:
        return {"unique_ratio": 0.0, "coverage": 0.0}
...
def _evaluate_all(
    ...
    *,
    ...
    # Pre-compute item popularity (count of interactions per item)
    item_popularity: Dict = data.train.groupby("item_id").size().to_dict()
...
def _generate_markdown_report(
    ...
    lines: list[str] = []
    lines.append("# Last.fm 1K Music Benchmark Results\n")
...
def run(*, smoke: bool = False) -> Dict[str, Any]:
    ...
    logger.info("=" * 70)
    logger.info("Last.fm 1K Music Benchmark -- %s mode", mode)
    logger.info("=" * 70)

    # Step 1: Download
    logger.info("[1/5] Downloading dataset...")
    lastfm_dir = download_and_extract()

    # Step 2: Preprocess
    logger.info("[2/5] Preprocessing...")
    data = preprocess(data_dir=lastfm_dir, seed=SEED)
    logger.info(
        "  users=%d  items=%d  train=%d  val=%d  test=%d  feat_dim=%d",
    ...
    logger.info("[3/5] Training engage simulator...")
    ...
    logger.info("=" * 70)
    logger.info(
        "Benchmark complete in %.1f seconds (%.1f minutes)",
        elapsed, elapsed / 60,
    )
    logger.info("=" * 70)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python benchmarks/music/run.py",
        description="Run the Last.fm 1K music benchmark suite.",
    )
    parser.add_argument(
        "--smoke",
        action="store_true",
