"""Listen-probability simulator for replay-based retention scoring.

Trains a small MLP on actual user-track interaction data from Last.fm 1K,
then serves as a "ground truth" oracle to simulate whether a user would
engage with (re-listen to) a recommended track.  Used by the benchmark
replay loop to evaluate recommenders on session-level retention metrics.

Since music data is purely implicit (listen events), the MLP is trained on
binary labels: positive = observed (user, track) interaction, negative =
randomly sampled (user, track) pair not in the training set.

Standalone smoke-test::

    python benchmarks/music/simulator.py --smoke-test
...
class _ClickMLP(nn.Module):
    """Two-layer MLP for listen/engage prediction.
    ...
    * Output: ``Linear(hidden_dim, 1) -> Sigmoid``
    ...
    def forward(
        ...
        """Return engage probabilities in [0, 1].
...
def _sample_negatives(
    ...
    """Sample negative (user, item) pairs not present in training data.

    For each positive interaction, sample *num_negatives* random items that
    the user did NOT interact with.

    Returns
    -------
    pd.DataFrame
        Combined positive and negative samples with columns:
    ...
    for _, row in train_df.iterrows():
        uidx = user_id_to_idx.get(row["user_id"])
        iidx = item_id_to_idx.get(row["item_id"])
        if uidx is not None and iidx is not None:
            user_item_set.add((uidx, iidx))
            pos_users.append(uidx)
            pos_items.append(iidx)
    ...
    neg_users: list[int] = []
    neg_items: list[int] = []
    for uidx in pos_users:
        for _ in range(num_negatives):
            # Keep sampling until we find a non-observed pair
            for _attempt in range(10):
                iidx = int(rng.choice(all_items))
                if (uidx, iidx) not in user_item_set:
                    break
            neg_users.append(uidx)
            neg_items.append(iidx)

    neg_labels = np.zeros(len(neg_users), dtype=np.float32)

    samples = pd.DataFrame({
        "user_idx": pos_users + neg_users,
        "item_idx": pos_items + neg_items,
        "label": np.concatenate([pos_labels, neg_labels]),
    })

    logger.info(
        "Negative sampling: %d positive + %d negative = %d total samples",
        len(pos_users), len(neg_users), len(samples),
    )
    return samples
...
class ClickSimulator:
    """MLP-based engage probability model for replay evaluation.

    Trained on historical interactions with negative sampling.  Given a
    ``(user, item)`` pair, predicts the probability that the user would
    engage with (listen to) the track.
    ...
    def fit(
        ...
        """Train the MLP on historical interactions with negative sampling.

        Parameters
        ----------
        train_df : DataFrame
            Must contain columns ``user_id``, ``item_id``, ``label``.
            For implicit data, all labels are 1; negatives are sampled
            internally.
        user_id_to_idx : dict
        ...
        -------
        dict
            Training metrics: ``epoch_losses``, ``final_loss``, ``final_acc``.
        """
        # Seed everything
        torch.manual_seed(seed)
        np.random.seed(seed)

        # Sample negatives and build training data
        samples = _sample_negatives(
            train_df, user_id_to_idx, item_id_to_idx,
            num_negatives=num_negatives, seed=seed,
        )

        user_idxs = samples["user_idx"].to_numpy(dtype=np.int64)
        item_idxs = samples["item_idx"].to_numpy(dtype=np.int64)
    ...
    def click_prob(self, user_idx: int, item_idx: int) -> float:
        """Predict engage probability for a single (user, item) pair.
        ...
        feat = self._item_features[int(item_idx)].unsqueeze(0)
    ...
    def click_probs_batch(
        ...
        """Vectorised engage probabilities.
        ...
        -------
        np.ndarray[float]
            Predicted P(engage) for each pair.
        """
        if not self._fitted:
            raise RuntimeError(
                "ClickSimulator has not been fitted yet. Call .fit() first."
...
def replay_sessions(
    ...
    3. The simulator samples an engagement via ``Bernoulli(click_prob(user, item))``.
    4. If engaged, the session continues and the item is added to the seen set.
    5. If not engaged, the session ends.

    Parameters
    ----------
    simulator : ClickSimulator
        Trained engage oracle.
    recommender
    ...
    top_k : int
        ...
        per step.
    threshold : float
        Reserved for a deterministic variant. Not used in default
        stochastic mode.
    seed : int
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python benchmarks/music/simulator.py",
        description=(
            "Listen-probability simulator for the Last.fm 1K benchmark.  "
        ...
        epilog=(
            ...
            "\n"
            "    from benchmarks.music.simulator import ClickSimulator\n"
            "\n"
            "    sim = ClickSimulator(num_users=992, num_items=50000,\n"
...
def _smoke_test(seed: int = 42) -> None:
    ...
    metrics = sim.fit(
        train_df, u2idx, i2idx, epochs=5, lr=0.001,
        num_negatives=1, seed=seed,
    )
    ...
    class _RandomRecommender:
        ...
        def recommend(
            ...
            chosen = self._rng.choice(
                available, size=min(k, len(available)), replace=False,
            )
            return list(chosen)
