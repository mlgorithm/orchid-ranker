"""Tests for curriculum modules: DependencyGraph and ProgressionRecommender."""

from orchid_ranker.curriculum import DependencyGraph, ProgressionRecommender


class TestDependencyGraph:
    """Test DependencyGraph for curriculum dependency modeling."""
    ...
    def test_from_dict_deserialization(self):
        ...
        edges = [('a', 'b'), ('b', 'c')]
        original = DependencyGraph(edges=edges)
    ...
    def test_summary_non_empty_graph(self):
        ...
        assert 'DependencyGraph' in summary or 'DependencyGraph' in summary
...
class TestProgressionRecommender:
    """Test ProgressionRecommender for pedagogical skill recommendations."""
    ...
    def test_initialization_invalid_difficulty_raises(self):
        ...
        graph = DependencyGraph([('a', 'b')])
        try:
            ProgressionRecommender(graph, difficulty_map={'a': 1.5})
            assert False, "Should raise ValueError"
        except ValueError as e:
    ...
    def test_recommend_respects_prerequisites(self):
        ...
        graph = DependencyGraph([('a', 'c'), ('b', 'c'), ('c', 'd')])
        rec = ProgressionRecommender(graph)
    ...
    def test_recommend_with_difficulty_ordering(self):
        ...
        graph = DependencyGraph([('a', 'b'), ('a', 'c'), ('a', 'd')])
    ...
    def test_filter_candidates_valid(self):
        ...
        rec = ProgressionRecommender(graph)
        candidates = ['b', 'c']
        filtered = rec.filter_candidates(candidates, completed={'a'})
